...
select userID, sum(price * amount) as 'ID별 구매금액'
  from buyTbl
-- where sum(price * amount) > 1000 일반적인 값을 비교 할 떄는 WHERE절 쓸 수 있음
 group by userID
 having sum(price * amount) > 1000 -- 집계함수나 GROUP BY에 연관 되어있으면 HAVING절을 써야함
  ORDER BY	sum(price * amount) DESC; -- 이건 쓸 수 있음

-- roll up / cube : 
select num, groupName, sum(price * amount) as '구매금액',
	   grouping_ID(groupName, num) -- 중간합계
  from buyTbl
 group by rollup (groupName, num); -- 총합계>> select에 쓴건 group by에 다 나와야 함 + 넣는 순서에 따라 결과값이 다르다

 -- userID, groupName가지고 CUBE 다차원 합계 (합계를 따로냄)
